...
- **Relation**：テーブルの中身そのもの。Attributeの集合とTupleの集合からなる。行に並び順はなく、同じTupleを2つ持つこともできない
- **Attribute**：列。名前と型を持つ
- **Tuple**：1件の行。各Attributeの名前と値を対応させたもの。値は位置ではなく名前で参照する
- **Schema**：Relationが持つAttributeの集合。列名、型、NULL許可の定義そのもの
...
Schema(列の定義)とTuple(値の並び)を別の型として持たせれば、列数や型の組み合わせがテーブルごとに違っても、同じ型で表現できます。

ただし、SchemaとTupleをどう実装するかは、関係モデル自体が決めることではありません。
関係モデルのTupleは、Attributeの名前をキーにして値を対応させたものであり、行の並び順も、値を取り出すときの位置も定義に含まれません。
このリポジトリでは、SQLエンジンを作るという目的に合わせて、列の並び順を保持するSchema(`Vec<Column>`)と、その並びに位置で対応する値の配列Tuple(`Vec<Value>`)という実装を選びます。
`SELECT *`の展開順や`INSERT INTO t VALUES (...)`の値の並びを、SQLは列の定義順どおりに扱う以上、列の順序をどこかで保持しておく必要があり、`Vec`による位置対応がもっとも素直な実装だからです。
`Value`は列の型を問わず格納できる列挙型にし、実際の型検査はSchemaとの突き合わせで行います。

なお、SQL自体も、関係モデルが定める「重複のないTupleの集合」という定義をそのまま採用しているわけではありません。
SQLのテーブルや問い合わせ結果は重複行を許すバッグ(多重集合)として扱われ、`DISTINCT`や一意制約を指定しない限り、同じ内容の行が複数残ります。
