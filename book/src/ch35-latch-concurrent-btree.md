...
`next()`をまたいでLatchを持ち越さない設計は、もう1つ別の帰結を持ちます。
`RangeScan`は、次の`next()`が来るたびにLeaf Pageを毎回読み直すため、「直前にどのエントリを返したか」を、ページ内の添字ではなく**値そのもの**(`(key, rid)`)で覚えておく必要があります(`crate::btree::ScanPosition`)。
添字は、他スレッドがその葉へ`insert`、`delete`するたびに指す意味が変わってしまうからです。
重複キーを持つエントリが並ぶ範囲では、この値そのものを使った位置の再特定にも同じ理由でひと工夫が要ります。
重複キーのエントリは`leaf_insert_position`が決める挿入順で物理的に並んでおり、`RecordId`の値順には並んでいません。
直前に返した`rid`が、次の`next()`が読み直した時点でまだそこにあれば、単にその次の物理位置から再開すれば済みます。
問題は、直前に返した`rid`自身が(たとえば他スレッドの`delete`によって)その間に消えていた場合です。
物理的な並びを頼りに「見つからなければ同じキーの範囲を通り過ぎた位置から再開する」と単純化すると、まだ返していない残りのエントリまで一緒に読み飛ばしてしまいます。
そこで`RecordId`に`Ord`を持たせ、重複キーの範囲を「`(key, rid)`の辞書式順序」という、挿入順とは独立した安定した全順序で走査します。
直前に返した`rid`が消えていても、消えていなくても、「その`rid`より大きい、範囲内で最小の`RecordId`」を毎回探し直せば、次に返すべきエントリを一意に再特定できます。

この全順序は、走査の**途中**(直前に返したエントリがある状態)だけでなく、走査の**開始**にも同じ規則で揃えなければなりません。
`range`が最初に読む位置(下限が`Unbounded`のとき、下限に一致するキーが無いとき、`Excluded`で1つ手前のキーを通り過ぎた直後)は、どれも二分探索で見つけた**物理的な**添字にすぎず、その添字が指すエントリの`RecordId`が、同じキーの範囲内で最小だとは限りません。
最初の1件が範囲内で最大の`RecordId`だった場合、「直前に返したエントリより`RecordId`が大きいものだけを次に返す」という規則のもとでは、残りの(実際にはもっと小さい)`RecordId`を1件も返せないまま範囲を読み終えたと誤判定してしまいます(`delete`が1件も絡まない、単なる`range`の呼び出しだけで再現します)。
そこで`RangeScan`は、走査の開始でも、重複キーの範囲を同じ葉の中で読み進める場合でも、`start_of_run_containing`という1つの関数を経由させ、「物理的にどこへ着地したかにかかわらず、そのキーの範囲内で`RecordId`が最小のエントリへ正規化する」という規則に統一しています。

この正規化が要るのは、走査の開始点だけではありません。
`ScanPosition::After`が今のキーの範囲を読み尽くし、次のキーへ移る場面(同じ葉の中で隣のキーへ進む場合と、そのキー自体が`delete`で消えていた場合の両方)も、「まだ1件も読んでいない新しいキーへ着地する」という点で走査の開始とまったく同じです。
ここも`start_of_run_containing`を経由し忘れると、次のキーの物理的に先頭のエントリが、その範囲内で最大の`RecordId`だったときに同じ理屈で残りを取りこぼします。
`locate_within_leaf`は、「新しいキーへ着地するすべての経路(`Start`の3境界と`After`のfallback、合わせて4か所)は必ず`start_of_run_containing`を通す」という不変条件を、呼び出し側のコメントとしてではなく、その4か所の戻り値の形そのもの(`start_of_run_containing(...)`の呼び出し以外の値を返す経路を残さない)で表しています。
同じキーの範囲内で前へ進むだけの`After`の通常の1歩(`min_rid_index_after`が`Some`を返す場合)だけがこの正規化の対象外であり、それはすでに`RecordId`の大小関係で正しく選ばれているからです。

### `next_leaf`をまたぐときは、探索条件そのものを引き継ぐ

`next()`が今の葉を読み尽くして`next_leaf`(第24章)へ移るとき、次の葉で何を探すかは「同じ葉の中で次のキーへ進む」場合とは別の注意が要ります。
`position`が`Start(下限)`(この葉では1件も見つからなかった)なら、同じ`下限`をそのまま次の葉でも使います。
`position`が`After(key, rid)`(すでにこのキーの一部を返した)なら、`Start(Bound::Included(key))`へ**変換**してから次の葉へ渡します。
`rid`による大小比較(`min_rid_index_after`)は、同じキーが**1つの葉の中で**物理的な挿入順に並んでいることに対する、その葉だけの重複解消手段だからです。
葉をまたいだ`RecordId`の大小には何の意味も無く(次の葉のエントリは、この葉のどの`RecordId`とも無関係に採番されています)、`After(key, rid)`をそのまま次の葉へ引き継ぐと、次の葉に残る同じキーの続きが「たまたま`rid`より小さい`RecordId`を持つ」というだけの理由で欠落します。

この区別を怠り、次の葉へ移るたびに`ScanPosition::Start(Bound::Unbounded)`(下限を一切課さない)へ一律差し替えていた版が、実際にこの教材のレビューで見つかったバグでした。
`BTree::lookup(K)`が葉Aを読んで一致するエントリが無いと判定し、`next_leaf`で葉Bへ移った直後、別のスレッドがBへキーK未満の新しいエントリを`insert`していると、`Start(Bound::Unbounded)`はBの物理的な先頭エントリを無条件に返し、上限Kを超えているかどうかだけで判定していたため、そのK未満のエントリを「Kと一致した」かのように返してしまいます。
`insert`の`unique`重複検査(次段落)はこの`lookup`の結果件数しか見ないため、実際には存在しないキーに対して`DbError::BTreeUniqueViolation`を誤って返していました。
`crate::btree::ScanPosition`のドキュメント(「Leaf間をまたぐ探索条件を弱めてはならない」)に、再現条件と直し方の詳細を残してあります。
...
この章の統合テストは、スレッドごとに互いに素なキー集合だけを挿入しており、この経路(同じキーへの競合)は突いていません。
ただし「互いに素なキー集合」であることは、並行アクセスからB+Treeを安全にするとは限りません。
上で説明した`next_leaf`をまたぐ探索条件のバグは、キーが互いに素であっても`lookup`(ひいては`unique`検査)を誤らせる形で実際にこの統合テストを失敗させ、レビューで発見されました。
