...
そこで`RangeScan`は、走査の開始と、重複キーを再開する`next_leaf`遷移のどちらも、`start_of_run_containing`という1つの関数を経由させ、「物理的にどこへ着地したかにかかわらず、そのキーの範囲内で`RecordId`が最小のエントリへ正規化する」という規則に統一しています。
