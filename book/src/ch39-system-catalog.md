...
けれどこのテーブルが実際に使っているデータページの枚数を測ってみると(測り方は本文「実測: VACUUMの有無でファイルサイズはどう変わるか」を参照)、1サイクルごとにほぼ倍々で増え続け、5サイクル目には334ページに達しています。
...
`VACUUM`を挟まない側は、サイクルを重ねるたびに使用ページがほぼ倍々で増え続けます。
